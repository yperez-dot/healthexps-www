...
## From Telegram (today, after this PR merges)
...
Railway `GITHUB_TOKEN` needs **Contents + Pull requests** write on this repo.

## Make Igor do that without being told (Railway)

This Cloud Agent cannot push `igor-config`. Apply `docs/igor-v2-publish-blog.patch` on branch `cursor/igor-v2-agent-cc2c`, merge, **redeploy Igor V2**. That adds `publish_blog` so Telegram calls this workflow (or writes files directly) when she says write/post.
